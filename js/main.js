...
btn.addEventListener("click", async function () {
  var res = await fetch(
    `https://api.weatherapi.com/v1/forecast.json?key=f00c6e3c7cda41049ef225602241106&q=${search.value}&days=3`
  );
...
(async function () {
  var res = await fetch(`https://api.weatherapi.com/v1/forecast.json?key=f00c6e3c7cda41049ef225602241106&q=07112&days=37`);
  finalRes = await res.json();
  console.log(finalRes);
  var d = new Date(`${finalRes.forecast.forecastday[0].date}`);
  var dayName = days[d.getDay()];
